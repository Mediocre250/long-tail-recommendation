...
def load_data_all(path="../data/ml100k/movielens_100k.csv", header=['user_id', 'item_id', 'rating', 'time'],
                  test_size=0.2, sep="\t"):
...
######################################################
    ...
    cold = np.where(sum_m == 0)                     #冷门item
    print('The shape of train is {}'.format(train.shape))
    print('The shape of sum_m is {}'.format(sum_m.shape))
    print('The shape of cold is {}'.format(cold))
    s = []
    for i, k in enumerate(sum_m):
        s.append((k, i))
    long_tail = sorted(s, reverse=True)
    hot=[x[1] for x in long_tail[0:200]]
    long_item=[x[1] for x in long_tail[200:400]]                #取前200个item为热门item，第200~400个为long-tail item
    print('The s is {}'.format(s))
    print('The hot is {}'.format(hot))
    print('The long_item is {}'.format(long_item))
    # hot=[x[1] for x in long_tail[0:500]]
    # long_item=[x[1] for x in long_tail[500:1000]]
######################################################
    train_data, test_data = train_test_split(df, test_size=test_size)           #按照8：2的比例划分训练、测试集
    train_data = pd.DataFrame(train_data)
    test_data = pd.DataFrame(test_data)
