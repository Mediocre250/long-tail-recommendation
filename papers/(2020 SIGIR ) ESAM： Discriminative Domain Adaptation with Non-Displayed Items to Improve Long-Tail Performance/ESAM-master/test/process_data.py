...
df_movie=pd.read_csv(path_movies, sep="::", header=None,names=['item_id','Title','Genres'], engine='python', encoding='utf-16')
...
hot = [x[1] for x in long_tail[0:800]]                                  #hot item 取前800，但其实并没有用到
long_item = [x[1] for x in long_tail[800:]]                             #long-tail item 取其余
# # x_id=[]
# # y=[]
# # for i in long_tail:
# #     x_id.append(i[1])
...
for i in df_rating.iterrows():
    ...
    neg_items[i[1][0]-1] = list(all_items - set(train_matrix.getrow(i[1][0]-1).nonzero()[1]))           #user没有选择的item
    list_of_random_items=random.sample(neg_items[i[1][0]-1], 2)
    item_append += list_of_random_items
    source_train[j]=(i[1][0],item_append)                                                               #一个正样本+两个负样本
    j=j+1
#     if j==100:
# # #
#        break
