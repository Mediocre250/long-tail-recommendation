...
class NeuMF_my_tail():
    def __init__(self, sess, num_user, num_item, learning_rate=0.5, reg_rate=0.01, epoch=500, batch_size=256,
        ...
        self.epochs = epoch                                         #轮次
        self.batch_size = batch_size                                #批次大小
        self.reg_rate = reg_rate                                    #正则率
        self.sess = sess                                            #tensorflow会话
        self.num_user = num_user                                    #user数量
        self.num_item = num_item                                    #item数量
        self.verbose = verbose                                      #不输出日志信息
        self.T = T                                                  #每次测试前的训练轮次
        self.display_step = display_step                            #显示进度前的迭代次数
        print("NeuMF.")

    def build_network(self, num_factor=10, num_factor_mlp=64, hidden_dimension=10, num_neg_sample=30):
        self.num_neg_sample = num_neg_sample                #采样数量
    ...
    def build_network_my(self, num_factor=10, num_factor_mlp=64, hidden_dimension=10, num_neg_sample=30):
        ...
        user_latent_factor = tf.nn.embedding_lookup(self.P, self.user_id)                                   #返回user embedding
        item_latent_factor = tf.nn.embedding_lookup(self.Q, self.item_id)
        mlp_user_latent_factor = tf.nn.embedding_lookup(self.mlp_P, self.user_id)
        mlp_item_latent_factor = tf.nn.embedding_lookup(self.mlp_Q, self.item_id)
###################################################################################################
...
###################################################################################################
        GMF = tf.multiply(user_latent_factor, item_latent_factor)                                           #user与item embedding相乘？



        user_feature=self.user_side(mlp_user_latent_factor)                                                 #经多层感知机处理后的user特征
        ...
        self.pred_long=tf.nn.sigmoid(tf.reduce_sum(tf.concat([GMF,5*tf.multiply(user_feature, long_item_feature)], axis=1), 1))         #给出预测概率
        # self.pred_y = tf.layers.dense(inputs=tf.concat([GMF, MLP], axis=1), units=1, activation=tf.sigmoid, kernel_initializer=tf.random_normal_initializer, kernel_regularizer= tf.contrib.layers.l2_regularizer(scale=self.reg_rate))



        #Pseudo label
        self.p1 = tf.reshape(tf.gather(self.pred_long, tf.reshape(tf.where(tf.less(self.pred_long, 0.2)), [-1, ])), [-1, 1])        #预测值小于0.2的切片
        self.p2 = tf.reshape(tf.gather(self.pred_long, tf.reshape(tf.where(tf.greater(self.pred_long, 0.8)), [-1, ])), [-1, 1])     #预测值大于0.8的切片
        self.tar1 = tf.maximum(0.0, tf.reduce_mean(
            -self.p1 * tf.log(tf.clip_by_value(self.p1, 0.005, 1)))) #/ self.batch_size                                             #取均值，但这个maximum什么鬼？？？还有为什么把概率压缩在0.005~1之间？？？
        self.tar2 = tf.maximum(0.0,
                               tf.reduce_mean(-self.p2 * tf.log(tf.clip_by_value(self.p2, 0.005, 1)))) #/ self.batch_size
        self.pseudo_loss=self.tar1+self.tar2                                                                                        #伪标签损失
        self.loss = - tf.reduce_sum(
            self.y * tf.log(self.pred_y + 1e-10) + (1 - self.y) * tf.log(1 - self.pred_y + 1e-10)) \
                    + tf.losses.get_regularization_loss() + self.reg_rate * (
        tf.nn.l2_loss(self.P) + tf.nn.l2_loss(self.Q) + tf.nn.l2_loss(self.mlp_P) + tf.nn.l2_loss(self.mlp_Q))                      #统计神经网络正则损失+交叉熵损失
        self.DAloss=tf.maximum(0.0001,KMMD(hot_item_feature,long_item_feature))                                                     #MMD方法下的领域偏移损失
        # self.DAloss=self.coral_loss(hot_item_feature,long_item_feature)
        # self.optimizer = tf.train.AdagradOptimizer(self.learning_rate).minimize(self.loss)
        # self.total_loss=self.loss+(3)*self.DAloss
        self.total_loss=self.loss                                                                                                   #这里的loss仅为Ls
...
##################################################
        self.neg_items = self._get_neg_items(train_data.tocsr())                #user没有选择的items
        self.test_users = set([u for u in self.test_data.keys() if len(self.test_data[u]) > 0])
##################################################
...
##################################################
    ...
    def train(self):
        ...
        for u in self.user:                                     #对于每个标签，从hot与long-tail items各自随机采样一个item（value为1）
        ...
        hot_item_append=[]
        long_item_append=[]
        for u in self.user:                         #对于每个user，从hot、long-tail、未选择items中各随机采样30个items，赋标签values为0
        ...
        print('The number of num_training is {}'.format(self.num_training))
        print('The total batch is {}'.format(self.total_batch))
